...
void CParseMatrice::PAMVerifierType() 
{
	...
	sRetour = PARSubString(sBuffer, 12, 7);
...
void CParseMatrice::PAMTraiterFichier(char * sChemin)
{
	...
	PAMParser.PARModifierChemin(sChemin);
	PAMParser.PAROuvrirFichier(sChemin);

	PAMParser.PAMVerifierType();
	...
	CMatrice<double> pMATMatrice = CMatrice<double>(3, 3);
